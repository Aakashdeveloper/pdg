...
(function(window){
  ...
  */
  ...
  //Drawing boxes with delay (Async) with box instance as paramters (JSON object)
  Main.prototype.drawWithDelay = function(box_info) {
  	var ref = this;
  	setTimeout(function() {
    	var box = new Box();
        box.style = box_info.style;
        box.position = box_info.position;
        box.dimension = box_info.dimension;
        ref.addBoxToStage(box);
    },box_info.delay);
  }
  
  //Initaite drawing sequence animation with delay
  Main.prototype.redrawAnimation = function() {
  	var all_boxes = Object.assign([], this.getBoxInstanceCollection());
    this.getBoxContainer().innerHTML = "";
    for(var i =0;i<all_boxes.length; i++){
    	this.drawWithDelay(all_boxes[i])
    }
  }
  ...
  Main.prototype.buildUI = function() {
    ...
    var redraw = this.createButton("Redraw", "REDRAW");
